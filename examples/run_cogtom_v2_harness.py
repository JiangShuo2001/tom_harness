"""Run the harness on CogToM — single-shot runtime with v2 skill/RAG/memory.
...
def process_one(runtime_factory, sample, timeout_sec: float = 180.0):
    ...
    try:
        ...
        result = runtime.answer_one(
            ...
            task_type=sample["metadata"].get("category"),
        )
...
def main():
    ...
    rag_grp = ap.add_argument_group("RAG retrieval (v2 — ToM rules)")
    ...
    logger.info("Selected %d samples across %d categories (offset=%d, limit=%s per category)",
                len(pool), len(cat_counts), args.offset, args.limit or "all")
    for c in sorted(cat_counts):
        logger.info("  %-40s %4d", c, cat_counts[c])
...
def _print_and_save_stats(results_path: Path, stats_path: Path, pool: list) -> None:
    ...
    logger.info("%-30s %4s %7s  %4s", "Category", "N", "Acc", "Err")
    logger.info("-" * 50)
    for c in sorted(stats["per_category"]):
        cs = stats["per_category"][c]
        logger.info("  %-28s %4d %6.1f%%  %4d", c, cs["total"], cs["accuracy"] * 100, cs["errors"])
    if stats.get("per_subcategory"):
        logger.info("")
        logger.info("%-55s %4s %7s  %4s", "Subcategory", "N", "Acc", "Err")
        logger.info("-" * 75)
        for sc in sorted(stats["per_subcategory"]):
            scs = stats["per_subcategory"][sc]
            logger.info("  %-53s %4d %6.1f%%  %4d", sc, scs["total"], scs["accuracy"] * 100, scs["errors"])
